...
public class PhoneBookSearchThread {
    private int count = 0;

    public static void main(String args[]) {
        List<PhoneBook> phoneBookList = new ArrayList<>();
        PhoneBookSearchThread phoneBookSearch = new PhoneBookSearchThread();
        phoneBookSearch.initPhoneBook(phoneBookList);
        long start = System.currentTimeMillis();
        for(int i=0; i<20000; i++) {
//            phoneBookSearch.searchPhoneBook("同学"+i, phoneBookList);
            SearchPhoneThread searchPhoneThread = new SearchPhoneThread("同学"+i, phoneBookList);
            Thread thread = new Thread(searchPhoneThread);
            thread.start();
        }
